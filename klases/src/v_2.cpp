...
int v_2(){
    ...
    if (pasirinkimas == 'G' || pasirinkimas == 'g') {
        ...
        auto startSortTime = std::chrono::high_resolution_clock::now();
            ...
            for (std::vector<Studentas>::size_type i = 0; i < studentai.size(); ) {
            ...
            for (const Studentas& studentas : studentai) {
                double galutinisBalas = studentas.skaiciuotiGalutiniBala(baloSkaiciavimoBudas);
                studentaiFile << std::setw(15) << std::left << studentas.getVardas() << std::setw(15) << std::left << studentas.getPavarde() << std::setw(15) << std::fixed << std::setprecision(2) << std::right << galutinisBalas << "\n";
            }
            studentaiFile.close();
