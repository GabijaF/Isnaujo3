...
int l_3(){
    ...
    if (pasirinkimas == 'G' || pasirinkimas == 'g') {
        ...
        while (std::getline(inFile, eilute)) {
            ...
            int pazymys;
            while (iss >> pazymys) {
                naujasStudentasL.addNd(pazymys);
            }

            if (!naujasStudentasL.getNd().empty()) {
                naujasStudentasL.setEgz(naujasStudentasL.getNd().back());
                naujasStudentasL.getNd().pop_back();
        ...
        auto startSortTime = std::chrono::high_resolution_clock::now();
            ...
            auto partitionPoint = std::stable_partition(studentai.begin(), studentai.end(), [&](const StudentasL& studentasL) {
            ...
            for (const StudentasL& StudentasL : vargsiukai) {
                double galutinisBalas = StudentasL.skaiciuotiGalutiniBala(baloSkaiciavimoBudas);
                vargsiukaiFile << std::setw(15) << std::left << StudentasL.getVardas() << std::setw(15) << std::left << StudentasL.getPavarde() << std::setw(15) << std::fixed << std::setprecision(2) << std::right << galutinisBalas << "\n";
                }
            ...
            for (const StudentasL& StudentasL : galvociai) {
                double galutinisBalas = StudentasL.skaiciuotiGalutiniBala(baloSkaiciavimoBudas);
                galvociaiFile << std::setw(15) << std::left << StudentasL.getVardas() << std::setw(15) << std::left << StudentasL.getPavarde() << std::setw(15) << std::fixed << std::setprecision(2) << std::right << galutinisBalas << "\n";
                }
            galvociaiFile.close();
    ...
    if (pasirinkimas == 'R' || pasirinkimas == 'r') {
        do {
            ...
            if (generuotiPazymius == 'T' || generuotiPazymius == 't') {
                ...
                for (int i = 0; i < NdKiekis; ++i) {
                    naujasStudentasL.addNd(generuotiNdBala());
                }
                naujasStudentasL.setEgz(generuotiEgzBala());
            } else {
                int pazymys;
                ...
                naujasStudentasL.setEgz(pazymys);
            }
        ...
        if (rikiavimas == 'Y' || rikiavimas == 'y') {
            ...
            for (const StudentasL& StudentasL : vargsiukai) {
                double galutinisBalas = StudentasL.skaiciuotiGalutiniBala(baloSkaiciavimoBudas);
                std::cout << std::setw(15) << std::left << StudentasL.getVardas() << std::setw(15) << std::left << StudentasL.getPavarde() << std::setw(15) << std::fixed << std::setprecision(2) << std::right << galutinisBalas << "\n";
            }
