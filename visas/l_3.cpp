    ...
    if (pasirinkimas == 'G' || pasirinkimas == 'g') {
        ...
        auto startSortTime = std::chrono::high_resolution_clock::now();
            ...
            for (const StudentasL& studentasL : galvociai) {
                double galutinisBalas = skaiciuotiGalutiniBala(studentasL, baloSkaiciavimoBudas);
                galvociaiFile << std::setw(15) << std::left << studentasL.vardas << std::setw(15) << std::left << studentasL.pavarde << std::setw(15) << std::fixed << std::setprecision(2) << std::right << galutinisBalas << "\n";
            }
            galvociaiFile.close();
